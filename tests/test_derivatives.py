...
def test_am_jacobian_saap():
    ...
    expected_m22_j = np.array([[4.0, 0.0], 
                               [3.0, 2.0]])
    np.testing.assert_allclose(result, expected_m22_j, rtol=1e-4)

def test_partial_derivative_saap_index_out_of_bounds():
    """Test that an invalid idx raises the specific AssertionError defined in the module."""
    def s_f(v2_x: np.ndarray) -> float:
        return v2_x[0]**2 + v2_x[1]**2
    
    v2_x = np.array([2.0, 3.0])
    
    # The array has length 2. Index 2 should trigger the assertion.
    with pytest.raises(AssertionError, match="The index is out of bounds."):
        partial_derivative_saap(s_f, v2_x, idx=2)

    # Index 5 should also trigger the assertion.
    with pytest.raises(AssertionError, match="The index is out of bounds."):
        partial_derivative_saap(s_f, v2_x, idx=5)


def test_divergence_saap_dimension_mismatch():
    """Test that a non-matching vector field (R^n -> R^m where n != m) raises an AssertionError."""
    # R^2 -> R^3 mapping
    ...
    with pytest.raises(AssertionError, match="The function must return a vector of the same dimension as the input."):
